...
function locate() {
  if (!navigator.geolocation) {
    showError('Geolocation Not Supported', 'Your browser does not support geolocation. Try a modern browser.');
    return;
  }
  showLoading();
...
async function fetchStations(lat, lng) {
  ...
  try {
    ...
    if (stations.length === 0) {
      showError('No Stations Found', `No gas stations found within ${radius} miles. Try increasing your search radius.`);
      return;
    }
...
function getFormattedPrice(station, grade) {
...
function renderStationList(sorted, grade) {
  ...
  stationList.innerHTML = sorted.map((station, i) => {
    const price = getPrice(station, grade);
    const formatted = price !== null ? `$${price.toFixed(2)}` : null;
    ...
    return `
      <div class="station-card">
        ...
        <div>
          ${formatted
            ? `<div class="station-price ${color}">${formatted}</div>
...
function renderCompareTable(sorted) {
  ...
  // Find min price per grade for highlighting
  const mins = {};
  grades.forEach(g => {
